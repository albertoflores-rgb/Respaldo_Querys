...
-- Área    : E-Catman
-- Versión : 1.4 | Ago 2026 — Ventas .com migradas a Sams_Ventas (ecom)
--
-- TABLAS CONFIRMADAS:
--   wmt-edw-prod.MX_WC_VM.SKU_DLY_POS             → ventas físicas diarias
--   wmt-mx-dl-controlledmgzn-prod.ecom.Sams_Ventas → ventas .com (fuente ACTUAL,
--       reemplaza MX_WC_MB_VM.SCAN + VISIT + STORE_TRANSACTION desde v1.4)
...
--   wmt-intl-cons-mx-users.adhoc_users.SAMS_MERCH_MX_CLUBES_INFO → dim. clubs (reemplaza Black_Bird.Catalogo_Clubes, sandbox ya no habilitado)
--   wmt-mx-dl-controlledmgzn-prod.Black_Bird.Catalogo_Cat_Subcat → dim. categorías (reemplaza sandbox)
--
-- NOTA .com (v1.4): Sams_Ventas ya viene a nivel orden/ítem, sin necesidad de
--   JOIN a VISIT/STORE_TRANSACTION. Piezas = sales_order_detail_commercial_sale_qty_base
--   ("sale_qty"). Venta $ = sales_order_detail_net_paid_orders_wo_shipping_amount_1
--   ("wo_shipping_amount"). Fecha = sales_order_detail_order_created_date.
--   Tienda = sales_order_detail_assigned_store_nbr (INTEGER). Item (join) =
--   sales_order_detail_item_id_short — VERIFICAR que sea equivalente a
--   ITEM_DESC.Old_NBR antes de confiar el join a ojos cerrados.
--
...
cte_ventas AS (
  ...
  LEFT JOIN  `wmt-intl-cons-mx-users.adhoc_users.SAMS_MERCH_MX_CLUBES_INFO` f ON v.STORE_NBR = f.CLUB_NBR
...
-- ------------------------------------------------------------
-- CTE 3: Ventas .com mensuales TY + LY
--   Fuente (v1.4): wmt-mx-dl-controlledmgzn-prod.ecom.Sams_Ventas
--   Reemplaza a MX_WC_MB_VM.SCAN/VISIT/STORE_TRANSACTION (v1.3 y anteriores).
--   Piezas   = sales_order_detail_commercial_sale_qty_base   ("sale_qty")
...
--              VERIFICAR que sea equivalente a ITEM_DESC.Old_NBR antes de
--              confiar el join a ojos cerrados -- son tablas de dominios
--              distintos (edw vs controlledmgzn).
--
--   Campos disponibles en Sams_Ventas, dejados aqui COMENTADOS por si se
--   necesita desglosar por canal / tienda / tipo de entrega / promocion:
--     -- s.sales_order_detail_header_channel_desc                    AS Canal,
--     -- s.sales_order_detail_assigned_store_name                    AS Tienda_Asignacion,
--     -- s.sales_order_detail_business_order_delivery_type_desc      AS Tipo_Entrega,
--     -- s.sales_order_detail_order_promotion_discount_desc          AS Promocion,
-- ------------------------------------------------------------
cte_ventas_com AS (
  SELECT
    s.sales_order_detail_assigned_store_nbr                AS CLUB_NBR,
    SAFE_CAST(s.sales_order_detail_item_id_short AS INT64)  AS ITEM_NBR,

    -- .com Ano Actual -- Piezas
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 1  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M01,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 2  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M02,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 3  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M03,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 4  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M04,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 5  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M05,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 6  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M06,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 7  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M07,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 8  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M08,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 9  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M09,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 10 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M10,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 11 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M11,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 12 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M12,
    -- .com Ano Pasado -- Piezas
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 1  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M01LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 2  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M02LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 3  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M03LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 4  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M04LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 5  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M05LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 6  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M06LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 7  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M07LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 8  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M08LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 9  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M09LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 10 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M10LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 11 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M11LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 12 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_commercial_sale_qty_base END), 0) AS COM_Pzas_M12LY,
    -- .com Ano Actual -- Pesos
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 1  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M01,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 2  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M02,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 3  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M03,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 4  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M04,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 5  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M05,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 6  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M06,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 7  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M07,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 8  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M08,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 9  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M09,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 10 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M10,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 11 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M11,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 12 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M12,
    -- .com Ano Pasado -- Pesos
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 1  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M01LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 2  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M02LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 3  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M03LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 4  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M04LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 5  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M05LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 6  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M06LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 7  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M07LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 8  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M08LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 9  AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M09LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 10 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M10LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 11 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M11LY,
    COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM s.sales_order_detail_order_created_date) = 12 AND EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN s.sales_order_detail_net_paid_orders_wo_shipping_amount_1 END), 0) AS COM_Pesos_M12LY

  FROM `wmt-mx-dl-controlledmgzn-prod.ecom.Sams_Ventas` AS s
  WHERE
    EXTRACT(YEAR FROM s.sales_order_detail_order_created_date) IN (
      EXTRACT(YEAR FROM CURRENT_DATE),
      EXTRACT(YEAR FROM CURRENT_DATE) - 1
    )
  ...
  GROUP BY
    s.sales_order_detail_assigned_store_nbr,
