...
-- Área    : E-Catman
-- Versión : 1.3 | Junio 2026  — Fix TYPE_CODE Sam's ('22'/'20' vs 'A'/'I' de WM)
--
-- TABLAS CONFIRMADAS:
...
cte_ventas_com AS (
  ...
  INNER JOIN `wmt-edw-prod.MX_WC_MB_VM.VISIT`      AS v
    ON  s.VISIT_NBR  = v.VISIT_NBR
    AND s.STORE_NBR  = v.STORE_NBR
    AND s.VISIT_DATE = v.VISIT_DATE
  INNER JOIN `wmt-edw-prod.MX_WC_MB_VM.STORE_TRANSACTION` AS t
    ON  v.VISIT_NBR  = t.VISIT_NBR
    AND v.VISIT_DATE = t.VISIT_DATE
    AND v.STORE_NBR  = t.STORE_NBR
  INNER JOIN `wmt-edw-prod.MX_WC_VM.ITEM_DESC`     AS i
    ON  s.ITEM_NBR   = i.ITEM_NBR
  WHERE
